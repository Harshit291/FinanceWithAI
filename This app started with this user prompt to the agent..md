This app started with this user prompt so this is the source of truth, so whenever going into the direction or add any new features in let me know before editing this prompt. Edit the prompt dynamically according to the project status and write the new and updated prompt in the new folder. I litreally want to track the prompt versions so that going ahead i have the memory of all the previous prompt to set the project directions.
# PROJECT INSTRUCTIONS — Finance with AI (FinAI)

> Paste this entire document into the **Project Instructions** field of your Claude Project. Every new chat in this project inherits these rules. Update the "Living Documents" section whenever architecture, scope, or stack decisions change.
