...
|---|---|---|---|
| `ANTHROPIC_API_KEY` | Auth for the Claude Messages API. | `lib/ai/llm.ts`, `services/app/pipeline/synthesis.py` | dev, prod |
| `LLM_SYNTHESIS_MODEL` | Model ID for the per-stock synthesis call (§5.6). Default: `claude-opus-4-7` (ADR-0004). Never hardcoded. | same as above | dev, prod |
| `LLM_CLASSIFIER_MODEL` | Model ID for the per-news classifier call (§5.4). Default: `claude-haiku-4-5` (ADR-0004). Never hardcoded. | `services/app/pipeline/classify.py` | dev, prod |
